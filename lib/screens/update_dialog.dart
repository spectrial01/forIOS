...
class _UpdateDialogState extends State<UpdateDialog> {
      ...
      final downloadResult = await updateService.downloadApkWithPath(
        ...
        (progress) {
          ...
          if (_autoInstall) {
            ...
            // Handle auto-install in a separate try-catch to avoid interfering with download error handling
            try {
              print('UpdateDialog: Starting auto-install for APK: $_apkPath');
              await _installUpdate();
              print('UpdateDialog: Auto-install completed successfully');
            } catch (e) {
              // Handle auto-install errors separately
              print('UpdateDialog: Auto-install failed: $e');
              setState(() {
                _isInstalling = false;
                _downloadError = 'Auto-install failed: $e. You can still install manually using the Install button.';
              });
            }
          } else {
  ...
  Future<void> _installUpdate() async {
    print('UpdateDialog: _installUpdate() called with APK path: $_apkPath');
    
    if (_apkPath == null || _apkPath == 'Downloaded successfully') {
      print('UpdateDialog: Invalid APK path, cannot install');
      setState(() {
    ...
    try {
      print('UpdateDialog: Setting _isInstalling = true');
      ...
      print('UpdateDialog: Calling updateService.installApk with path: $_apkPath');
      final success = await updateService.installApk(_apkPath!);
      print('UpdateDialog: installApk returned: $success');
